...
const turnSchema = z.object({
  ...
  startTime: z.string().min(3, 'La hora de inicio es requerida'),
  endTime: z.string().min(3, 'La hora de fin es requerida'),
})
...
const TurnModal: React.FC<TurnModalProps> = ({
  isOpen,
  onClose,
  turn,
  onSubmit,
  ...
  useEffect(() => {
    if (turn) {
      setValue('name', turn.name);
      setValue('startTime', turn.startTime);
      setValue('endTime', turn.endTime);
    } else if (isOpen) {
      reset();
    }
  }, [turn, isOpen, setValue, reset])
