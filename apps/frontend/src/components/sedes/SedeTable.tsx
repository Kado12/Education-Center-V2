...
interface SedeTableProps {
  ...
  onToggleStatus: (sede: Sede) => void;
  onDelete: (sedeId: number) => void;
  isToggling: boolean;
  isDeleting: boolean;
}
  ...
  if (sedes.length === 0) {
    return (
      <div className="p-8 text-center text-gray-500">
        ...
        <p className="mt-2">No hay sedes registradas</p>
      </div>
    );
  }
            ...
            <tr key={sede.id} className="hover:bg-gray-50">
                ...
                <div className="flex items-center">
                  <div className="ml-4">
                    ...
                    <div className="text-sm text-gray-500">
                      Creado: {new Date(sede.createdAt).toLocaleDateString()}
                    </div>
                  </div>
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-gray-900">{sede.code}</div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                {sede.isActive ? (
              ...
              <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div className="flex items-center justify-end space-x-2">
                  ...
                  <button
                    onClick={() => onToggleStatus(sede)}
                    className={`${sede.isActive ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900'
                      } p-1`}
