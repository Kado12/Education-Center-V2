...
interface ProcessTableProps {
  ...
  onToggleStatus: (process: Process) => void;
  ...
  if (isLoading) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="mt-4 text-gray-500">Cargando procesos...</p>
      </div>
    );
  }
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {processes.map((process) => (
            <tr key={process.id} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-gray-900">{process.code}</div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                {process.isActive ? (
              ...
              <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div className="flex items-center justify-end space-x-2">
                  ...
                  <button
                    ...
                    className={`${process.isActive ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900'
                      } p-1`}
                    title={process.isActive ? 'Desactivar' : 'Activar'}
                  >
                    {process.isActive ? (
